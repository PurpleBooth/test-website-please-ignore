 ...
 *
 */
export default function useIpfs(ipfs: IPFS, cmd: string, opts?: unknown[]): null|unknown {
  const [res, setRes] = useState<null|unknown>(null)
  useEffect(() => {
...
async function callIpfs(ipfs: IPFS, cmd: string, setRes: ((value: unknown) => void), ...opts: unknown[]): Promise<void> {
  if (!ipfs) return null
  console.log(`Call ipfs.${cmd}`)
  const ipfsCmd: (...options: unknown[]) => Promise<unknown> = dotProp.get(ipfs, cmd)
